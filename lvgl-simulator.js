...
`;
    ...
    preprocessYAML(text) {
        // !secret someValue -> quoted placeholder
        text = text.replace(/!secret\s+(\S+)/g, '"__secret_$1__"');
        // !lambda "..." -> placeholder
        text = text.replace(/!lambda\s+"[^"]*"/g, '"__lambda__"');
        text = text.replace(/!lambda\s+'[^']*'/g, '"__lambda__"');
        // !lambda |- or !lambda | (block scalar) - collapse the whole block
        // This replaces the tag and its indented block with a placeholder
        text = text.replace(/!lambda\s+\|[-]?\s*\n([ \t]+[^\n]*\n)*/g, '"__lambda__"\n');
        return text;
    }
